...
Source: https://raw.githubusercontent.com/safishamsi/graphify/v1/skills/graphify/skill.md
...
Three things it does that a single ephemeral agent session cannot reliably do alone:
...
This step has two parts: **structural extraction** (deterministic, free) then **semantic extraction** (agent-assisted, costs tokens).
...
Use the runtime's parallel subagent facility here when it is available and allowed. In Claude this is the Agent tool; in Codex this may be parallel subagents; in Gemini this may be the equivalent delegated-agent feature. If no parallel delegation is available, process the chunks sequentially and tell the user that the run will be slower.
...
**Step B2 - Dispatch extraction workers**

When the runtime supports parallel delegation, dispatch all extraction workers in one batch - one worker per chunk. If you make one worker call, wait, then make another, you are doing it sequentially and defeating the purpose. If the runtime does not support this, process each chunk yourself and keep the same JSON schema.

Concrete example for 3 chunks:
```
[worker call 1: files 1-15]
[worker call 2: files 16-30]
[worker call 3: files 31-45]
```
...
For example, an MCP client config can point at the generated graph:
```json
{
  "mcpServers": {
    "graphify": {
...
What's inside:
  ...
  graphify-out/GRAPH_REPORT.md  - full audit report, also readable in chat
...
- Images (.png/.jpg/.webp) → downloaded, the active agent's vision model extracts on next run
